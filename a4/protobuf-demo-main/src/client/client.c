...
static error_t parse_opt(int key, char *arg, struct argp_state *state)
{
	...
	switch (key)
	{
		...
		case 't':
			...
			break;
		case 'd':
			arguments->display_string = atoi(arg);
			break;
		case 'r':
...
int main(int argc, char *argv[])
{
	...
	struct arguments args = 
	{
		...
		.port = 0,
		.timeout = 10,
		.display_string = DISPLAY_STRING__HELLO_WORLD,
		.repeat_count = 1
	};
	...
	if (args.server == 0 || args.port == 0)
	{
		...
		argp_help(&argp, stderr, ARGP_HELP_STD_HELP, "client");
		return 1;
	}
	...
	StringRepeatRequest request = STRING_REPEAT_REQUEST__INIT;

	// set the request fields
	request.requested_string = args.display_string;
	request.repeat_count = args.repeat_count;

	// pack the message
	size_t request_len = string_repeat_request__get_packed_size(&request);
	void *request_buf = malloc(request_len);
	string_repeat_request__pack(&request, request_buf);

	// -------- send --------
	ret = sendto(sock, request_buf, request_len, 0, (struct sockaddr *)&server_addr, sizeof(struct sockaddr_in));
	if (ret < 0)
	{
		free(request_buf);
	...
	assert(ret == request_len);

	// since we finished sending, we can free the buffer
	free(request_buf);
	...
	StringRepeatResponse *response = string_repeat_response__unpack(NULL, actual_recv_len, udp_recv_buf);
	...
	if (response->repeated_string == NULL && response->error_message == NULL)
	{
		EPRINTLN("Response is missing fields!");
		string_repeat_response__free_unpacked(response, NULL);
		return 1;
	}
